    ...
    <div class="info">
      <el-form class="form" :model="form">
        ...
        <el-row>
          <el-col :span="6">
            <el-form-item >
              <el-input class="radius" :class="{'require': !form.contact}" v-model="form.contact" :placeholder="$t('order.name')"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="12">
        ...
        <el-row v-if="showSpareInfo">
          <el-col :span="6">
            <el-form-item >
              <el-input class="radius" :class="{'require': !form.spare_contact_name}" v-model="form.spare_contact_name" :placeholder="$t('order.name')"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item class="ml15">
      ...
      <div class="left">
          <div class="order-info" v-if="dayOrderData.daies">
            <div class="title">
                <img v-if="form.type==2" src="../../assets/images/airportdropoff.png" alt="">
                <img v-if="form.type==1" src="../../assets/images/airport2.png" alt="">
                <span class="fz16 color-333 fw550 ml10">{{dayOrderData.cityName}} {{dayOrderData.daies.length}} {{$t('order.days-charters')}}</span>
            </div>
            <div class="trap-info">
                <p><img src="../../assets/images/reservationtime.png" alt=""> <span class="fz14 color-333 ml10">{{dayOrderData.param.start_date}} ~ {{dayOrderData.param.end_date}}</span></p>
                <p v-for="(item,index) in dayOrderData.daies" :key="index">
                    <img v-if="index < dayOrderData.daies.length-1"  src="../../assets/images/reservationstart.png" alt="" >
                    <img v-if="index == dayOrderData.daies.length-1"  src="../../assets/images/reservation-destination.png" alt="" > 
                    <span class="fz14 color-333 fw550 ml10">{{item.day}}</span>
                    <span class="fz14 color-333">{{item.type}}</span>
                </p>
            </div>
            <div class="car flex-between" v-if="dayOrderData.car">
                <div>
                    <p class="fz14 color-333 fw550">{{dayOrderData.car.name}}</p>
                    <p class="max-w">
                        <span class="fz14 color-333" v-if="lang=='zh'">{{dayOrderData.car.model}}或同级别车型&nbsp;&nbsp;可容纳 </span>
                        <span class="fz14 color-333" v-if="lang=='en'">{{dayOrderData.car.model}} corolla or similar </span><br>
                        <img class="ml5" src="../../assets/images/passenger.png" alt=""> <span class="fz14 color-green">*{{dayOrderData.car.people}}&nbsp;</span> 
                        <img src="../../assets/images/luggage.png" alt=""> <span class="fz14 color-green">*{{dayOrderData.car.luggage}}</span> 
                    </p>
                </div>
                <div>
                    <img class="car-img" v-lazy="dayOrderData.car.img" alt="">
                </div>
            </div>
            <div>
                <div class="flex-between server-info p-relative">
                  <div>
                      <p>{{$t('order.Daily-charter')}}</p>
            ...
            <div class="flex-between total">
                <div>
                    ...
                    <p class="fz14 color-333 fw550">
                      <span v-if="lang == 'en'">{{dayOrderData.extra_ffee.rebate_rate*10}}% </span>{{$t('order.commission')}}
                      <span v-if="lang == 'zh'">{{dayOrderData.extra_ffee.rebate_rate*10}}%</span>
                      <img class="wh14" src="../../assets/images/detail.png" alt="">
                    </p>
                </div>
                <div>
                    <p class="fz22 color-333 fw550">{{currency}}{{totalPrice.toFixed(2)}}</p>
                    <p class="fz16 color-333 fw550">{{currency}}{{(totalPrice.toFixed(2) * dayOrderData.extra_ffee.rebate_rate).toFixed(2)}}</p>
                </div>
            </div>
          </div>
...
export default {
  ...
  mounted(){
    let orderData = JSON.parse(sessionStorage.getItem('dayOrderData'));
    this.dayOrderData = orderData;
    this.carPrice = Number(orderData.car.price);
    this.currency = orderData.currency;
    console.log(this.dayOrderData)
  ...
  methods: {
      next() {
        ...
        let param = this.dayOrderData.param;
        let param2 = {
          ...
          car_id: this.dayOrderData.car.id,
        ...
        this.$axios.post(this.lang+'/day-charter/make', param).then(res=> {
      ...
      changePerson(){
        ...
        this.childrenPrice = this.people.children > 1 ? this.dayOrderData.extra_ffee.children_seat : 0;
        this.countTotalPrice();
    },
    changeLuggage() {
